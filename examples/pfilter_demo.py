...
from tpath.pquery._filter import pfilter, pfind, pcount
...
def demo_complex_queries(temp_dir):
    ...
    complex_results = list(pfilter(
        from_=[temp_dir / "src", temp_dir / "test"],
        query=lambda p: (p.suffix == ".py" and p.size.bytes < 50) or "test" in p.name
    ))
