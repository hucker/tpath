...
def demo_fluent_from_methods():
    ...
    temp_dir, src_dir, test_dir, docs_dir, logs_dir, config_dir = (
...
def demo_complex_fluent_queries():
    ...
    file_info = (
        ...
        .select(
            lambda p: (p.name, p.size.bytes, "large" if p.size.bytes > 100 else "small")
        )
    )
...
def demo_convenience_methods():
    ...
    query = PQuery().from_(src_dir).from_(test_dir).where(lambda p: p.suffix == ".py")
