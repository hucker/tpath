...
def demo_enhanced_constructor():
    ...
    query = PQuery()
    print(f"   Before execution: start_paths={query.start_paths}, query_func={query._query_func}")
    ...
    query = PQuery(
        from_=temp_dir,
        recursive=False,
        where=lambda p: p.size.bytes > 100
    )
    files = query.files()
    ...
    query = (PQuery(from_=temp_dir, where=lambda p: p.suffix == ".py")
             .where(lambda p: p.suffix == ".md"))  # Override to markdown
    files = query.files()
    ...
    query_with_dirs = PQuery(from_=temp_dir, recursive=False, 
                            where=lambda p: True)  # Include everything
...
def demo_lazy_vs_eager():
    ...
    query_override = (PQuery(from_=temp_dir, recursive=False)
...
if __name__ == "__main__":
    ...
    print(f"\n✨ Key Features:")
    print(f"  • PQuery() - Default constructor with sensible defaults")
    print(f"  • PQuery(from_=path) - Set starting directory")  
    print(f"  • PQuery(recursive=True/False) - Control recursion")
    print(f"  • PQuery(where=lambda p: ...) - Set initial filter")
    print(f"  • Lazy initialization - defaults applied when query runs")
    print(f"  • Fluent override - methods can override constructor parameters")
    print(f"  • Default where function - lambda p: p.is_file() (files only)")
    
    print(f"\n🗂️  Test files created in: {temp_dir}")
    print(f"    Feel free to explore the directory structure!")