...
def demo_pquery_basics():
    ...
    py_sizes = pquery(from_=src_dir).where(lambda p: p.suffix == ".py").select(lambda p: (p.name, p.size.bytes))
...
def demo_pquery_advanced():
    ...
    large_or_py = pquery(from_=temp_dir).where(
        lambda p: p.size.bytes > 500 or p.suffix == ".py"
    ).files()
...
def demo_pquery_select():
    ...
    unique_exts = set(ext for ext in extensions if ext)  # filter out empty
    print(f"  {', '.join(sorted(unique_exts))}")
    
    # Extract complex data
    print("\n3. File info tuples:")
    file_info = pquery(from_=logs_dir).where(lambda p: p.suffix == ".log").select(
        lambda p: (p.name, p.size.bytes, p.readable)
    )
    ...
    all_files = pquery(from_=temp_dir).where(lambda p: True).select(
        lambda p: (p.suffix, p.size.bytes)
    )
