...
def test_pquery_fluent_vs_convenience_equivalence(tmp_path):
    ...
    # Fluent API
    fluent_files = list(PQuery().from_(dir1).from_(dir2).recursive(True).where(lambda p: p.suffix == ".txt").files())
    
    # Convenience function
    convenience_files = list(pquery(from_=[dir1, dir2], recursive=True).where(lambda p: p.suffix == ".txt").files())
...
def test_pquery_complex_fluent_example(tmp_path):
    ...
    large_py_or_test_files = list(PQuery()
                             .from_(src_dir)
                             .from_(test_dir) 
                             .recursive(True)
...
def test_pquery_error_handling_fluent(tmp_path):
    ...
    (tmp_path / "subdir").mkdir()  # This should be filtered out by default where function
