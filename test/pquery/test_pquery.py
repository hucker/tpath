...
def test_pquery_basic_functionality(tmp_path):
    ...
    small_files = list(
        pquery(from_=tmp_path).where(lambda p: p.size.bytes < 100).files()
    )
    assert len(small_files) == 2  # small.txt and test.py
...
def test_pquery_select_functionality(tmp_path):
    ...
    suffixes = list(
        pquery(from_=tmp_path).where(lambda p: True).select(lambda p: p.suffix)
    )
...
def test_pquery_multiple_paths(tmp_path):
    ...
    txt_files = list(
        pquery(from_=[dir1, dir2]).where(lambda p: p.suffix == ".txt").files()
    )
    assert len(txt_files) == 2

    # Test select with multiple paths
    names = list(
        pquery(from_=[dir1, dir2]).where(lambda p: True).select(lambda p: p.name)
    )
    assert len(names) == 4
