...
def test_integration_basic_workflow():
    ...
    try:
        ...
        assert ctime.age.seconds >= 0
        assert mtime.age.seconds >= 0
        assert atime.age.seconds >= 0
...
def test_integration_file_operations():
    ...
    try:
        ...
        # Test size comparison using fromstr
        min_size = Size.fromstr("50B")
        medium_files = [
            f for f in test_files
            if f.size.bytes > min_size
        ]
...
def test_integration_custom_base_time():
    ...
    try:
        ...
        if abs(age.hours) < 24:  # File is "newer" than 24 hours from base
            print(f"✅ File is newer than 24 hours from reference")
...
if __name__ == "__main__":
    ...
    test_integration_pathlib_compatibility()
    test_integration_custom_base_time()
